...
var FluidNav = {
    ...
    resizePage: function (size, animate, callback, currentPage) {
        if (size) { var new_size = size; } else { var new_size = $(".page.current").height() + 40; }
        if (!callback) { callback = function () { }; }

        // Additions for iFrame responsiveness (resets it's width)
        if (!currentPage) { currentPage = $(".page.current"); }
        var iFrames = currentPage.find("iframe");
        if (iFrames.length > 0) {
            var spaceWidth = $("div#page_wrapper").width(); // value set in "media.queries.css", mobile value is 320px
            var msg = "found iframe with width: " + iFrames[0].width + ", current space width: " + spaceWidth;
            //alert(msg);
            if (spaceWidth < 500) { // arbitrary value, but must be between mobile and next size up values (320 & 768)
                iFrames[0].width = spaceWidth;
            } else {
                iFrames[0].width = 555; // the "max-width" attribute only works at 550 and above.  So, 555 is the min width we can make the iFrame in the large-size.
            }
        }
